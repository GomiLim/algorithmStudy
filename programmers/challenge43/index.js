...
function solution(nums) {
  const getPokemon = new Set(nums);
  const getCount = nums.length / 2;
  return Array.from(getPokemon).length > getCount ? getCount : Array.from(getPokemon).length
}

function solution2(nums) {
  const count = nums.length / 2
  const newArray = [...new Set(nums)].length
  return newArray < count ? newArray : count;
}
