...
/*
문제 설명
배달 앱을 이용하려면 유저는 로그인 → 장바구니에 음식 담기 → 주문 과 같은 순서를 거칩니다. 
이때 서버는 다음과 같은 일을 합니다.
...
장바구니에 담은 음식이 없으면 거부
서버에 저장된 유저 정보 infos와 한 사람의 행동을 담은 배열 actions가 매개변수로 주어질 때, 
각 행동이 허용되었으면 true,
 거부되었으면 false를 담아 return 하도록 solution 함수를 완성해주세요.
...
const user = infos;
let userLogin = false;
let userAddProduct = false;

let checkAction = actions.map((action, idx) => {
  //로그인 확인
  if (!userLogin) {
    for(let i = 0; i < infos.length; i++) {
      if(`LOGIN ${user[i]}` === action) {
        userLogin= true;
        return true;
      } else {
        userLogin= false;
    ...
    if(action.indexOf('ADD') > -1) {
      ...
      if(userAddProduct) {
        if (action.indexOf('ORDER') > -1) {
          userAddProduct=false;
          return true;
        } else {
          return false;
        }
...
let infos = ["kim password", "lee abc"];
